...
public class TriProfile extends Profile {
    private final double max_velocity, acceleration, distance, t_half, t1, start_velocity, end_velocity, cap_velocity;
    
    public TriProfile(final double v_max, final double accel, final double dist, final double start_vel, final double end_vel){
        max_velocity = v_max;
        acceleration = accel;
        distance = dist;
    ...
    public double getVelocityAtTime(final double time){
        double v_exp;
        if(time > t1) {
        	v_exp = 0;
        }
    ...
    public double getDistAtTime(final double time){
        double dist = 0;
        if(time > t1) {
        	dist = distance;
        }
    ...
    public ArrayList<Double> getDistances(){
        final ArrayList<Double> distArr = new ArrayList<>();
        for(double i = 0.0; i < getFinalTime(); i += 0.1){
            distArr.add(getDistAtTime(i));
        }
        return distArr;
    }
